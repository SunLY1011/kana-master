<template>
    <div class="practice-area">
        <DiffSelector />
        <p class="question-label">{{ directionLabel }} · {{ diffLabel }}</p>
...
const direction = computed(() => route.params.direction === 'kata-to-hira' ? 'kata-to-hira' : 'hira-to-kata')
const isHiraToKata = computed(() => direction.value === 'hira-to-kata')
...
const diffLabel = computed(() => ({
    basic: '初级(清音)', intermediate: '中级(+浊音)', advanced: '高级(全部)'
}[store.difficulty]))

const directionLabel = computed(() => isHiraToKata.value ? '平假名→片假名' : '片假名→平假名')
